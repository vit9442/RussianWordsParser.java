...
public class RussianWordsParser implements Parser{
    ...
    public String mostFrequentWord() {
        ...
        Integer maxWordNumber = Integer.MIN_VALUE;

        for (Map.Entry<String, Integer> frequentWord : wordsFrequency.entrySet()) {
            if (maxWordNumber < frequentWord.getValue()) {
                maxWordNumber = frequentWord.getValue();
                Word = frequentWord.getKey();
            }
        }
        return ("Слово " + Word + " встречается - " + maxWordNumber + " раз");
    }
...
// возвращает информацию в виде строки о самом часто встречаемом слове.
    public double averageFrequency(){
        ...
        Integer sum = 0;
            for (Map.Entry<String, Integer> frequentWord : wordsFrequency.entrySet()) {
                sum += frequentWord.getValue();

        }
            return (double)sum/wordsFrequency.size();
    }
}
